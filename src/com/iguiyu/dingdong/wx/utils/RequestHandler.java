...
public class RequestHandler {
    ...
    public String genPackage(SortedMap<String, String> packageParams) throws UnsupportedEncodingException {
        ...
        while(it.hasNext()) {
            Entry packageValue = (Entry)it.next();
    ...
    public boolean createMd5Sign(String signParams) {
        ...
        String tenpaySign;
        while(it.hasNext()) {
            Entry enc = (Entry)it.next();
            sign = (String)enc.getKey();
            tenpaySign = (String)enc.getValue();
            if(!"sign".equals(sign) && null != tenpaySign && !"".equals(tenpaySign)) {
                sb.append(sign + "=" + tenpaySign + "&");
            }
        }

        String enc1 = TenpayUtil.getCharacterEncoding(this.request, this.response);
